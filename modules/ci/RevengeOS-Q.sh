...
Usage: \`/ci RevengeOS-Q [arguments]\`
...
ci_message() {
	if [ "$CI_MESSAGE_ID" = "" ]; then
		CI_MESSAGE_ID=$(tg_send_message "$CI_CHANNEL_ID" "CI | RevengeOS 3.2 (Q)
Device: $CI_DEVICE

Status: $1" | jq .result.message_id)
	else
		tg_edit_message_text "$CI_CHANNEL_ID" "$CI_MESSAGE_ID" "CI | RevengeOS 3.2 (Q)
Device: $CI_DEVICE
...
if [ "$CI_DEVICE" != "" ] && [ -d "$CI_MAIN_DIR/RevengeOS-Q" ]; then
	ci_message "Starting..."
	if [ "$CI_MESSAGE_ID" != "" ]; then
		cd "$CI_MAIN_DIR/RevengeOS-Q"
		ci_message "Setting up environment..."
		. build/envsetup.sh
		export SKIP_ABI_CHECKS=true
		ci_message "Lunching..."
		lunch revengeos_${CI_DEVICE}-userdebug
		CI_LUNCH_STATUS=$?
		if [ $CI_LUNCH_STATUS = 0 ]; then
			...
			if [ $CI_BUILD_STATUS = 0 ]; then
				if [ "$CI_ENABLE_GDRIVE_UPLOAD" = "true" ] && [ "$(command -v gupload > /dev/null 2>&1 && echo 0)" = "0" ]; then
					...
					CI_UPLOAD_LINK=$(gupload out/target/product/$CI_DEVICE/RevengeOS-*.zip | grep "https://drive.google.com/open?id=" | sed "s/[][]//g" | tr -d '[:space:]')
