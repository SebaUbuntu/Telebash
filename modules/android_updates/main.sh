...
module_revengeos() {
	if [ "$(tg_get_command_arguments "$@")" != "" ]; then
		local DEVICE_CODENAME="$(tg_get_command_arguments "$@")"
		...
		local REVENGEOS_DEVICE_JSON="$(curl https://raw.githubusercontent.com/RevengeOS-Devices/official_devices/r10.0/$DEVICE_CODENAME/device.json | jq .)"
